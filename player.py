...
class Player(pygame.sprite.Sprite):
    ...
    RESET_POWERUP_TIMER_IN_TICKS = 200
    ...
    JELLY_POWERUP_TIMER_IN_TICKS = 100

    SURGE_MOVE_SPEED = 9
    STUNNED_MOVE_SPEED=0
    ...
    def stop_movement(self):
        if self.speed_power == self.INVINCIBLE_POWERUP:  # Seahorse speed powerup
            ...
            self.speed_x, self.speed_y = self.STUNNED_MOVE_SPEED, self.STUNNED_MOVE_SPEED
    ...
    def collide_with_jellyfish(self):
        self.speed_power = self.SPEED_STUN
        self.size_score = 0
        self.speed_x, self.speed_y = self.STUNNED_MOVE_SPEED, self.STUNNED_MOVE_SPEED
        self.speed_time_left = self.JELLY_POWERUP_TIMER_IN_TICKS
    ...
    def get_speed_timer_text(self, font):
        ...
        elif self.speed_power == self.SPEED_STUN:
    ...
    def predator_eat_player_collision(self, enemy_object):
        if self.lives > 0:
            self.lives -= 1  # Reduce lives on collision
            
            # Activate the invincibility powerup for 1 second (60 ticks, assuming 60 FPS)
            self.star_power = self.INVINCIBLE_POWERUP
            self.powerup_time_left = self.RESET_POWERUP_TIMER_IN_TICKS
            self.reset_position()  # Optionally reset position after losing a life

        else:
            self.game_over = True  # End game if no lives left

    def reset_position(self):
    # Reset position and other state if needed after losing a life
        self.pos = [SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 + 100]
        self.rect.topleft = (self.pos[0], self.pos[1])
        self.star_power = self.INVINCIBLE_POWERUP
        self.powerup_time_left = self.RESET_POWERUP_TIMER_IN_TICKS
